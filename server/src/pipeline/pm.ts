...
export class PmGateError extends Error {
  constructor(msg: string) { super('PM gate error: ' + msg); }
}
...
export async function issueProceed(
  projectId: string,
  stage: number,
  db: SupabaseClient
): Promise<void> {
  ...
  if (error) throw new PmGateError('Failed to set pm_proceed_at: ' + error.message);
...
export async function issueLocked(
  ...
  if (error) throw new PmGateError('Failed to set LOCKED: ' + error.message);
...
export async function handleFounderAnswer(
  projectId: string,
  stage: number,
  questionId: string,
  answer: string,
  ...
  if (insertError) throw new PmGateError('Failed to store founder answer: ' + insertError.message);
  ...
  if (!stageRun) throw new PmGateError('Stage run not found for stage ' + stage);

  const questions: { id: string }[] = stageRun.questions_json ?? [];
