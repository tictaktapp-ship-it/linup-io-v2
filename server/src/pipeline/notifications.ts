...
async function getProjectOwner(projectId: string, db: SupabaseClient): Promise<{ userId: string | null; email: string | null; name: string | null }> {
  ...
  if (!project?.created_by) return { userId: null, email: null, name: project?.name ?? null };
...
async function sendEmail(to: string, subject: string, body: string): Promise<void> {
  ...
  if (!apiKey) { console.warn('[notifications] RESEND_API_KEY not set — skipping email'); return; }
  try {
...
// ---------------------------------------------------------------------------
export async function sendProviderOutage(projectId: string, stage: number, db: SupabaseClient): Promise<void> {
