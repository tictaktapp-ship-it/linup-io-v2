...
export interface PltOutput {
  stage: number;
  stageSummaryForFounder: string;  // plain English, no jargon
  whatHappenedThisStage: string;   // 2-3 sentence recap
  keyDecisionsPlain: string[];     // decisions in plain language
  assumptionsPlain: string[];      // assumptions in plain language
  questionsForFounder: IgQuestion[]; // max 5, A/B/C/D format
  hasQuestions: boolean;
  openItemsForFounder: string[];   // CoS open items in plain language
}
...
export async function run(
  ...
  stage: number,
  cosOutput: CosOutput,
  igQuestionsForFounder: IgQuestion[],
  db: SupabaseClient
): Promise<PltOutput> {
