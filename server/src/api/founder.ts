...
export async function founderRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/api/founder/answer', { preHandler: requireAuth }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { projectId, stage, questionId, answer } = request.body as {
      projectId: string;
      stage: number;
      questionId: string;
      answer: string;
    ...
    const { data: membership } = await supabase
      ...
      .eq('user_id', userId)
      .eq('organisation_id',
        supabase.from('projects').select('organisation_id').eq('id', projectId).single()
      )
      .maybeSingle();
    ...
    const result = await handleFounderAnswer(
      projectId,
      stage,
      questionId,
      answer,
