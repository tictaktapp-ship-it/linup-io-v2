...
export class StuckMemberError extends Error {
    constructor(memberId, stage, iterations) {
...
async function runIc(memberId, projectId, stage, groupSummaries, db) {
    ...
    while (iteration <= 3) {
        ...
        if (!validation.passed) {
            ...
            if (sfErr)
                console.error('[runIc] SCHEMA_FAIL insert error:', sfErr.message);
...
export async function runStage(projectId, stage, db) {
    try {
        // 1. PM pre-stage gate
        ...
        for (const group of vpAnalysis.executionSequence) {
            ...
            const icResults = await Promise.all(group.members.map((mid) => runIc(mid, projectId, stage, allGroupSummaries, db)));
        ...
        if (!fidelityResult.passed)
            throw new Error('Fidelity check failed: ' + fidelityResult.failedChecks.join(', '));
        // 6. Acceptance Test
        ...
        if (ig1Result.hold) {
            ...
            if (holdRecord.deadlocked) {
                await notifications.sendDeadlockNotification(projectId, stage, ig1Result, db);
                return;
            }
        ...
        if (ig2Result.hold) {
            ...
            if (holdRecord.deadlocked) {
                await notifications.sendDeadlockNotification(projectId, stage, ig2Result, db);
                return;
            }
            await notifications.sendHoldNotification(projectId, stage, ig2Result, db);
            return;
        }
        // CHECKPOINT 2
