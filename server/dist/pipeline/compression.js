...
export class CompressionValidationError extends Error {
    constructor(missing) { super('Compression missing constraints: ' + missing.join(', ')); }
}
export class CompressionSizeError extends Error {
    constructor(count) { super('Compression output too large: ' + count + ' words (max 500)'); }
}
export class CompressionError extends Error {
    constructor(stage, reason) { super('Compression failed for stage ' + stage + ': ' + reason); }
}
...
function buildCompressionPrompt(consolidation, rejectionReason) {
...
function validateAbstractCompleteness(abstract, consolidation) {
    const missing = [];
...
export async function compressStage(projectId, stage, consolidation, db) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
        ...
        try {
            ...
            if (attempt === 2)
                throw new CompressionError(stage, rejectionReason);
            continue;
        }
        // Size check
        ...
        if (wc > 500) {
            ...
            if (attempt === 2)
                throw new CompressionSizeError(wc);
            continue;
        }
        // Completeness check
        ...
        if (!validation.passed) {
            ...
            if (attempt === 2)
                throw new CompressionValidationError(validation.missingConstraints);
            continue;
        }
        ...
        if (error)
            throw new CompressionError(stage, 'DB insert failed: ' + error.message);
