...
const MEMBER_REQUIRED_SECTIONS = {
...
// --- validate ---
export async function validate(memberId, content) {
...
function getMemberRequiredSections(memberId) {
    if (MEMBER_REQUIRED_SECTIONS[memberId])
        return MEMBER_REQUIRED_SECTIONS[memberId];
    const stagePrefix = memberId.split('-')[0];
    return MEMBER_REQUIRED_SECTIONS[stagePrefix] ?? [];
}
// --- registerMemberSections ---
export function registerMemberSections(memberId, sections) {
