...
function getPdfFilename(slug, version) {
    const date = new Date().toISOString().split('T')[0];
    return slug + '_specification_v' + version + '_' + date + '.pdf';
}
function getZipFilename(slug) {
    const date = new Date().toISOString().split('T')[0];
    return 'linup_' + slug + '_' + date + '.zip';
}
async function assembleSpecData(projectId, stages, db) {
...
export async function generateSpecPdf(projectId, stages, db) {
    ...
    const coverPage = ce(Page, { style: styles.coverPage }, ce(View, { style: styles.coverTitle }, ce(Text, null, projectName + ' — Engineering Specification')), ce(View, { style: styles.coverMeta }, ce(Text, null, 'Generated by LINUP v2 — ' + new Date().toLocaleDateString('en-GB'))));
    const stagePages = data.consolidations.map((c) => ce(Page, { style: styles.page }, ce(View, { style: styles.h2 }, ce(Text, null, 'Stage ' + c.stage + ' — ' + (c.consolidation_json?.stageName ?? ''))), ce(View, { style: styles.body }, ce(Text, null, JSON.stringify(c.consolidation_json, null, 2)))));
    const doc = ce(Document, null, coverPage, ...stagePages);
    return renderToBuffer(doc);
}
async function generateRtmCsv(projectId, db) {
    const { data } = await db.from('rtm_entries').select('req_id, stage, status, notes').eq('project_id', projectId).order('stage');
    const rows = (data ?? []).map((r) => r.req_id + ',' + r.stage + ',' + r.status + ',' + (r.notes ?? '').replace(/,/g, ';'));
    return 'req_id,stage,status,notes\n' + rows.join('\n');
}
export async function generateAppPackageZip(projectId, db) {
    const archiver = (await import('archiver')).default;
    const chunks = [];
    const output = new Writable({ write(chunk, _enc, cb) { chunks.push(chunk); cb(); } });
    ...
    await new Promise((resolve, reject) => { output.on('finish', resolve); output.on('error', reject); archive.on('error', reject); });
    return Buffer.concat(chunks);
}
export async function uploadAndRecord(projectId, zipBuffer, db) {
...
//# sourceMappingURL=zip-generator.js.map