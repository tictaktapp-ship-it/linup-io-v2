...
export function getVpIdForStage(stage) {
    const vpId = VP_BY_STAGE[stage];
    if (!vpId)
        throw new Error('No VP configured for stage: ' + stage);
    return vpId;
}
...
export async function analyse(vpId, vpSystemPrompt, projectId, stage, db) {
    ...
    const userContent = 'PROJECT: ' + JSON.stringify(project?.identity_json ?? {}) +
        '\n\nPRIOR STAGE ABSTRACTS: ' + JSON.stringify((abstracts ?? []).map((a) => a.abstract_json)) +
    ...
    if (!report.executionSequence || report.executionSequence.length === 0) {
        ...
        const memberIds = (stageMembers ?? []).map((m) => m.member_id).filter((id) => !id.startsWith('P05-') && !id.startsWith('L-'));
        if (memberIds.length === 0)
            throw new Error('VP analysis returned empty executionSequence and no IC members found in DB for stage ' + stage);
...
export async function reviewIcOutput(vpId, vpSystemPrompt, memberId, icContent, groupReviewSummaries, projectId, stage, db) {
...
export async function reviewGroup(vpId, vpSystemPrompt, groupId, icContents, projectId, stage, db) {
...
export async function consolidate(vpId, vpSystemPrompt, projectId, stage, groupSummaries, db) {
