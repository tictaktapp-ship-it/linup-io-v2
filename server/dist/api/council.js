...
import { handleConciergeMessage, handlePisMessage, confirmIdeaBrief, getCouncilStatus, getPhase05Status, confirmFeatureCharter, handleConditionalResubmit, } from '../pipeline/council.js';
// ── Phase 0 Council API (Doc 8D Phase 9, Doc 5 Screen 6) ─────────────────────
// All endpoints require auth. Project membership verified on each request.
export async function councilRoutes(fastify) {
    // Helper: verify caller is a member of this project and return project row
    async function assertMember(projectId, userId, organisationId) {
    ...
    fastify.post('/api/council/confirm-brief', { preHandler: requireAuth }, async (request, reply) => {
        ...
        confirmIdeaBrief(body.projectId, body.ideaBrief).catch((err) => {
    ...
    fastify.post('/api/council/resubmit-conditional', { preHandler: requireAuth }, async (request, reply) => {
        ...
        handleConditionalResubmit(body.projectId, body.answers).catch((err) => {
