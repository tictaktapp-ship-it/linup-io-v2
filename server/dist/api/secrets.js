...
function encryptPayload(secrets) {
    const encrypted = {};
...
function resolveConfiguredFlags(row) {
    if (!row)
        return { supabase: false, stripe: false, email: false, push: false, openrouter: false, stripe_connect: false };
    return {
...
function decryptField(row, field) {
    const val = row[field];
    if (!val || typeof val !== 'string')
        return '';
    try {
        return decryptSecret(Buffer.from(val, 'base64'));
    }
    catch {
        return '';
    }
}
export async function secretsRoutes(fastify) {
    // GET /api/secrets/status/:project_id
    fastify.get('/api/secrets/status/:project_id', { preHandler: requireAuth }, async (request, reply) => {
    ...
    fastify.post('/api/secrets/test', { preHandler: requireAuth }, async (request, reply) => {
        const { project_id, service } = request.body ?? {};
        if (!project_id || !service)
            return reply.status(400).send({ error: 'project_id and service are required' });
        const { id: userId } = request.profile;
        const isMember = await assertProjectMember(project_id, userId);
        if (!isMember)
            return reply.status(403).send({ error: 'Access denied' });
        ...
        const { data: row, error: fetchErr } = await supabase
            ...
            .single();
        if (fetchErr || !row)
            return reply.send({ success: false, detail: 'No credentials saved for this service yet.' });
        const r = row;
        let result;
        try {
            switch (service) {
                case 'supabase': {
                    ...
                    if (!url || !key) {
                        result = { success: false, detail: 'Supabase credentials not fully saved.' };
                        break;
                    }
                    ...
                    if (error) {
                        result = { success: false, detail: 'Connection failed: ' + error.message };
                        break;
                    }
                ...
                case 'stripe': {
                    ...
                    if (!sk) {
                        result = { success: false, detail: 'Stripe secret key not saved.' };
                        break;
                    }
                ...
                case 'resend': {
                    ...
                    if (!apiKey) {
                        result = { success: false, detail: 'Resend API key not saved.' };
                        break;
                    }
                ...
                case 'openrouter': {
                    ...
                    if (!apiKey) {
                        result = { success: false, detail: 'OpenRouter API key not saved.' };
                        break;
                    }
                    ...
                    if (!res.ok) {
                        result = { success: false, detail: 'OpenRouter returned ' + res.status };
                        break;
                    }
    ...
    fastify.post('/api/secrets/generate-env', { preHandler: requireAuth }, async (request, reply) => {
        ...
        const { data: row, error: fetchErr } = await supabase
            ...
            .single();
        if (fetchErr || !row)
            return reply.status(404).send({ error: 'No secrets found for this project' });
        const r = row;
        const d = (f) => decryptField(r, f);
        const frontendEnv = [
    ...
    fastify.patch('/api/secrets/update', { preHandler: requireAuth }, async (request, reply) => {
        request.log = request.log.child({ body: '[REDACTED]' });
        const body = request.body;
