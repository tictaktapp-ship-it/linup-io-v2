...
export async function pipelineRoutes(fastify) {
    ...
    fastify.post('/api/pipeline/run/:projectId/:stage', { preHandler: requireAuth }, async (request, reply) => {
        ...
        const { projectId, stage: stageStr } = request.params;
        const stage = parseInt(stageStr, 10);
        if (isNaN(stage) || stage < 0 || stage > 12)
        ...
        const { data: stageRun } = await supabase
            ...
            .eq('stage', stage)
            .single();
        const runnableStatuses = ['PENDING', 'ERROR', 'HOLD', 'LOCKED', 'API_ERROR'];
        ...
        if (upsertErr)
            return reply.status(500).send({ error: 'Failed to queue stage: ' + upsertErr.message });
    ...
    fastify.get('/api/pipeline/status/:projectId/:stage', { preHandler: requireAuth }, async (request, reply) => {
        ...
        const { data, error } = await supabase
            ...
            .single();
        if (error || !data)
            return reply.status(404).send({ error: 'Stage run not found' });
    ...
    fastify.post('/api/pipeline/pause/:projectId', { preHandler: requireAuth }, async (request, reply) => {
        ...
        if (!running)
            return reply.status(409).send({ error: 'No stage currently running.' });
    ...
    fastify.post('/api/pipeline/resume/:projectId', { preHandler: requireAuth }, async (request, reply) => {
        ...
        if (!paused)
            return reply.status(409).send({ error: 'No paused stage found.' });
