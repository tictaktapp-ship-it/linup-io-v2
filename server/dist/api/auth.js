...
function checkSignupRateLimit(ip) {
    ...
    if (entry.count >= 3)
        return false; // Doc 3: max 3 signup attempts per IP per hour
    entry.count++;
    return true;
}
export async function authRoutes(fastify) {
    ...
    fastify.post('/api/auth/setup-totp', async (request, reply) => {
    ...
    fastify.post('/api/auth/verify-totp', async (request, reply) => {
        const { userId, token } = request.body;
        const { data: userData, error: userError } = await supabase.auth.admin.getUserById(userId);
        if (userError || !userData.user) {
            return reply.status(404).send({ error: 'User not found.' });
        }
        const pendingSecret = userData.user.user_metadata?.['totp_secret_pending'];
        if (!pendingSecret) {
    ...
    fastify.post('/api/auth/verify-2fa', async (request, reply) => {
        const { userId, token } = request.body;
        const { data: userData, error: userError } = await supabase.auth.admin.getUserById(userId);
        if (userError || !userData.user) {
            return reply.status(404).send({ error: 'User not found.' });
        }
        const totpSecret = userData.user.user_metadata?.['totp_secret'];
        if (!totpSecret) {
