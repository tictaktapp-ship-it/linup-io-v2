...
export async function oauthRoutes(fastify) {
    fastify.post('/api/auth/oauth/callback', async (request, reply) => {
        const { access_token, provider, provider_id } = request.body;
        ...
        const authMethod = provider.toUpperCase();
        ...
        const sessionToken = fastify.jwt.sign({ sub: userId, email, two_factor_verified: true, auth_method: authMethod }, { expiresIn: '7d' });
        return reply
