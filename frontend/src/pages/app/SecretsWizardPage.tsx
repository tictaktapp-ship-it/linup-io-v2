...
function TestButton({ service, projectId, onResult }: { service: string; projectId: string; onResult: (r: TestResult) => void }) {
  ...
  const run = async () => {
    setLoading(true);
    try {
      const res = await apiFetch('/api/secrets/test', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ project_id: projectId, service }) });
...
export default function SecretsWizardPage() {
  ...
  useEffect(() => {
    if (!projectId) return;
    apiFetch('/api/secrets/status/' + projectId, { credentials: 'include' })
  ...
  const saveStep = async (stepName: string, secrets: Record<string, string>) => {
    ...
    try {
      const res = await apiFetch('/api/secrets/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ project_id: projectId, step: stepName, secrets }) });
  ...
  const generateEnv = async () => {
    setSaving(true); setError(null);
    try {
      const res = await apiFetch('/api/secrets/generate-env', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ project_id: projectId }) });
      const data = await res.json();
